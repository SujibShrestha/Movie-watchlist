...
export const MovieSidebar = ({ movie, open, onOpenChange }: MovieSidebarProps) => {
      ...
      <DialogContent className="sm:max-w-md w-full right-0 fixed top-0 h-full p-6 bg-background overflow-auto shadow-lg flex flex-col">
        <DialogHeader className="mb-4">
          ...
          <DialogDescription className="text-sm text-muted-foreground">
            ...
            <span className="text-blue-400 font-semibold">{movie.voteAverage || movie.vote_average.toFixed(1)}</span>
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="mt-6 flex items-center gap-2">
          <span className="text-xl font-bold">{movie.voteAverage || movie.vote_average.toFixed(1)}</span>
          <div className="flex gap-1">
            {Array.from({ length: 5 }).map((_, i) => (
              <Star
                key={i}
                size={16}
                className={i < Math.round(movie.voteAverage || movie.vote_average.toFixed(1) / 2) ? "text-yellow-400" : "text-muted-foreground"}
              />
            ))}
          </div>
        </div>
